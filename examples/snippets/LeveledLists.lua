...
--
-- Form references work like this:
--   "EditorID"            -> looked up by EditorID
--   ("Plugin.esm", "000123")  -> form in a specific plugin, by hex local formID
--                              (light plugins use the 0xFFF-masked ID)
--   { "Plugin.esm", "000123" } -> same pair, as a table value
--
-- Reference cheat sheet (all items are also available on LeveledList):
--   lua_patcher.getForm(ref)                -> Form (formId, editorId, name, type, identifier)
...
-- 1) Grab a leveled list by plugin + local formID
local chest = lua_patcher.leveledList("LuaPatcherExample.esp", "00001000")
if chest == nil then
	lua_patcher.warn("Example: form 00001000 in LuaPatcherExample.esp not found, skipping")
	return
end
...
chest:add("LuaPatcherExample.esp", "00002000", 1, 1)                          -- plugin + formID pair
