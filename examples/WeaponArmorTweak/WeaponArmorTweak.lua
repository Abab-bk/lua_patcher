...
--
-- Solves two common problems that LeveledList patching alone cannot:
--
--  1) Missing material keywords — many modded weapons/armors have no
--     WeapMaterial*/ArmorMaterial* keyword, so they never benefit from
--     smithing perks (Steel Smithing, etc.), tempering bonuses, or perk
--     conditions like HasKeyword(WeapMaterialDaedric). This script detects
--     the gap and injects the correct keyword based on damage/rating tier.
--
--  2) Unbalanced stats — modded gear often has outlier damage/armorRating/
--     weight/value (e.g. a Glass-tier dagger with Iron-tier damage, or a
--     Heavy chest with 100 armorRating at level 1). Optionally rebalances
--     them to vanilla linear tiers and caps Heavy weight for encumbrance.
--
-- Runs at kDataLoaded on pristine data, idempotent. All changes are via
...
--
-- User config (outside mod, never overwritten):
--   Data/SKSE/Plugins/LuaPatcher/Config/WeaponArmorTweak.lua
--   Must `return { ... }`. Missing file → defaults below.
--   See WeaponArmorTweak_Config.lua in this folder.
...
do
    ...
    if lua_patcher.tryLoadConfig then
        ...
        if ok and type(res) == "table" then loaded = res
        elseif not ok then print("WeaponArmorTweak: tryLoadConfig failed: "..tostring(res)) end
    end
    if loaded then
        ...
        print(string.format("WeaponArmorTweak: no user config, defaults fixKeywords=%s rebalanceW=%s rebalanceA=%s",
            tostring(CONFIG.fixMissingMaterialKeywords), tostring(CONFIG.rebalanceWeaponDamage), tostring(CONFIG.rebalanceArmorRating)))
    end
end
    ...
    if not VANILLA[weap.plugin] and weap.playable and not weap.enchantment then
        ...
        if CONFIG.fixMissingMaterialKeywords and not hasAnyMaterialKeyword(weap) then
            ...
            if kw then
                ...
                if ok then
                    fixedKw = fixedKw + 1
                    print(string.format("WeaponArmorTweak: weapon %s dmg=%d -> added %s", weap.identifier, weap.damage, kw.editorId))
                end
            end
        end
        ...
        if CONFIG.rebalanceWeaponDamage then
            ...
            if weap.damage > tier.maxDmg + 2 then
                ...
                print(string.format("WeaponArmorTweak: rebalanced weapon %s %d -> %d (tier %s)", weap.identifier, old, weap.damage, tier.mat))
            end
    ...
    if not VANILLA[armor.plugin] and armor.playable and not armor.enchantment then
        ...
        if CONFIG.fixMissingMaterialKeywords and not hasAnyMaterialKeyword(armor) then
            ...
            if kw then
                ...
                if ok then
                    fixedKw = fixedKw + 1
                    print(string.format("WeaponArmorTweak: armor %s [%.1f %s] -> added %s", armor.identifier, armor.armorRating, armor.armorType, kw.editorId))
                end
            end
        end
        -- cap heavy weight
        if CONFIG.capHeavyWeight and armor.armorType == "Heavy" and armor.weight and armor.weight > CONFIG.maxHeavyWeight then
            ...
            capped = capped + 1
            print(string.format("WeaponArmorTweak: capped Heavy armor %s weight %.1f -> %.1f", armor.identifier, old, armor.weight))
        end
        ...
        if CONFIG.rebalanceArmorRating then
            ...
            if armor.armorRating > tier.maxRating + 5 then
                ...
                print(string.format("WeaponArmorTweak: rebalanced armor %s %.1f -> %.1f (tier %s)", armor.identifier, old, armor.armorRating, tier.mat))
            end
        end
    end
end
