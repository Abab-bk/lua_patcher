...
namespace LuaPatcher
{
	...
	{
		sol::usertype<LuaEncounterZone> type =
			a_lua.new_usertype<LuaEncounterZone>("EncounterZone", sol::base_classes, sol::bases<LuaForm>(),
				sol::meta_function::index, sol::readonly_property(UnknownPropertyGetter<LuaEncounterZone>),
				sol::meta_function::to_string, [](const LuaEncounterZone& a_zone) {
		...
		type["minLevel"] = sol::property(
			[](const LuaEncounterZone& a_zone) { return static_cast<lua_Integer>(ToZone(a_zone)->data.minLevel); },
			[](LuaEncounterZone& a_zone, lua_Integer a_value) {
				ToZone(a_zone)->data.minLevel =
					static_cast<std::int8_t>(std::clamp(a_value, lua_Integer{ -1 }, lua_Integer{ 100 }));
			});

		type["maxLevel"] = sol::property(
			[](const LuaEncounterZone& a_zone) { return static_cast<lua_Integer>(ToZone(a_zone)->data.maxLevel); },
			[](LuaEncounterZone& a_zone, lua_Integer a_value) {
				ToZone(a_zone)->data.maxLevel =
					static_cast<std::int8_t>(std::clamp(a_value, lua_Integer{ -1 }, lua_Integer{ 100 }));
			});
